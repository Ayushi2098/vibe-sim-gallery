...
files.forEach(file => {
    ...
    // Remove Header Block
    // Matches <div style={{ display: 'flex' ... }}> ... <h3 ...> ... <InfoTooltip ...> ... </div>
    // Being careful with nested divs
    ...
    if (regex.test(content)) {
        ...
        // Try alternate pattern without InfoTooltip wrapper div (if any)
        const altRegex = /<div style={{ display: 'flex', alignItems: 'center'.*?}}>\s*<h3.*?<\/h3>\s*<InfoTooltip.*?<\/div>/s;
        if (altRegex.test(content)) {
            content = content.replace(altRegex, '');
            fs.writeFileSync(filePath, content);
            console.log(`Processed ${file} (Alt Pattern)`);
        }
    }
});
